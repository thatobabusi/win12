...
- **上游同步（2026-06-24）** — 合并了 **#852**（设置 → Windows 更新：基于 Tauri 的真实
  GitHub 发布更新检查——原生版本比对 +「获取最新发布」链接）。Web 版保持惰性并明确标记为不可用。
  与本分支的 i18n 协调：新增 16 个 `setting.upd.*` 字符串并翻译到全部 5 种语言。同时补齐了
  延后的 3 个「关于」发行说明键——所有语言现为**每个 585 个键**。**有意跳过** #851（llama-guard——
  它会把 Copilot 聊天发送到第三方端点、URL 缺少协议头而失效，且未本地化）以及上游的语言子模块更新
  （本分支使用自己的 `win12-locales` 仓库，采用它会丢失茨瓦纳语）。
