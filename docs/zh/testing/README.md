...
**示例覆盖范围：**
- i18n 翻译逻辑（`i18n.test.js`，基于固定数据）
- **真实语言文件校验**（`lang-files.test.js`）—— 读取实际的
  `public/lang/lang/*.properties`，断言全部 5 种语言（英语、en-US、
  简体/繁体中文、**茨瓦纳语**）的完整键一致性、无空值、占位符完整，
  且英语/茨瓦纳语文件中不混入中文。这是防止茨瓦纳语落后的守卫。
- 窗口管理器操作
- 语言规范化

**端到端（Playwright）** 覆盖桌面/应用启动、语言切换（含 **茨瓦纳语**
及现已可用的简体中文）、关于应用的标签页（上游 #845）以及重启/关机页面。
Playwright 配置在端口 8123 上提供 `public/`（避免常见的 3000 端口冲突）。
